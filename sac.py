...
class SAC:
    ...
    def __init__(self):
        ...
        self.gammas = np.array([[args.gamma**i] for i in range(args.n_step_loss + 2)], dtype=np.float32)       
    ...
    def compute_target_q(self,                 
        ...
        for t1 in range(T): # T: 10.
            ...
            target_q_value[:, t1, :-1] = reward_sum + (self.gammas[1]**tn) * next_q[:, tn] # (b, q_dim). 

            ent_sum = np.sum(ent[:, t1:t1+n] * self.gammas[1:n+1, 0], axis=1)  # (b,)                    
            target_q_value[:, t1, -1] = ent_sum + (self.gammas[1]**tn) * next_ent[:, tn] # (b,).             
    ...
    def act(self, observation):                
        ...
        if(self.i % 4 == 0):            
            ...
            action = self.action_rescale(tf.math.tanh(mean)[0, 0].numpy())
            
            # action = self.action_rescale(self.act_sampler(obs_vec)[0, 0]) #.numpy() # (22,) 


            self.prev_action = action
            # self.prev_action = self.action_rescale(action)
    ...
    def save(self, dir_name, name):
        ...
        state_dict['actor'] = self.actor.get_weights()
        state_dict['critic1'] = self.critic1.get_weights()
        state_dict['critic2'] = self.critic2.get_weights()
        state_dict['critic1_tgt'] = self.critic1_tgt.get_weights()
        state_dict['critic2_tgt'] = self.critic2_tgt.get_weights()
    ...
    def load(self, path):
        ...
        self.actor.set_weights(state_dict['actor'])
        self.critic1.set_weights(state_dict['critic1'])
        self.critic2.set_weights(state_dict['critic2'])
        self.critic1_tgt.set_weights(state_dict['critic1_tgt'])
        self.critic2_tgt.set_weights(state_dict['critic2_tgt'])        
        ...
        # # print(f"state_dict['policy_net']: {state_dict['policy_net']}")
